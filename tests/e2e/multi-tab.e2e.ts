...
// subscribeBusToStore). Saving the profile in one tab fires a BroadcastChannel
...
test('a saved EAOS propagates across tabs: IDB load + live broadcast update', async ({
	...
	await expect(tabA.getByRole('heading', { level: 1, name: 'Transition Companion' })).toBeVisible();
