...
// corpus-v1.0.2.json); stamp today only on a real content change.
const { contentHash } = computeContentRevision(
	chunks.map((c) => ({ id: c.id, text: c.text })),
	''
);
const manifestPath = join(OUT_DIR, 'corpus-v1.0.2.json');
...
writeFileSync(join(OUT_DIR, 'corpus-v1.0.2.json'), JSON.stringify(manifest));
writeFileSync(join(OUT_DIR, 'corpus-v1.0.2.embeddings.bin'), Buffer.from(embeddingsBuffer));

const jsonBytes = statSync(join(OUT_DIR, 'corpus-v1.0.2.json')).size;
const binBytes = statSync(join(OUT_DIR, 'corpus-v1.0.2.embeddings.bin')).size;
...
const checkManifest = JSON.parse(readFileSync(join(OUT_DIR, 'corpus-v1.0.2.json'), 'utf8'));
const checkBuf = readFileSync(join(OUT_DIR, 'corpus-v1.0.2.embeddings.bin'));
